  ...
  <div class="p-currency-detail">
    ...
    <common-header class="header">
      <nav class="type-switch-bar">
        <span
          :class="{active: currAction === 'in'}"
          @click="currAction = 'in'">充值记录</span>
        <span
          :class="{active: currAction === 'out'}"
          @click="currAction = 'out'">提取纪录</span>
      </nav>
    </common-header>
...
import CurrencyDetailItem from "./CurrencyDetailItem.vue";
...
export default {
  ...
  computed: {
    ...
    currAction: {
      get() {
        return this.$route.query.action || "in";
      },
      set(val) {
        this.$router.replace({
